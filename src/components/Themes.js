...
const Themes = () => {
    ...
    useEffect(() => {
        ...
        axios.get('/get_parent_themes.php')
    ...
    const handleSearchSubmit = async (e) => {
        ...
        if (searchQuery.trim() !== '') {
            setIsLoading(true);
            try {
                const response = await axios.get(`/search_sets.php?query=${searchQuery}`);
            ...
            {searchResults.length === 0 ? (
                <div className="themes-list-container">
                    {parentThemes.map(theme => (
                        <button
                            key={theme.id}
                            className={`theme-button ${priorityThemes.includes(theme.id) ? 'highlighted' : ''}`}
                            onClick={() => handleThemeClick(theme.id)}
                        >
                            {theme.name}
                            {priorityThemes.includes(theme.id) && (
